...
============================
ERA5 Rajasthan — Comprehensive Visualization Batch. The 1:1 port of the
Tamil Nadu pipeline's 05d_plots_comprehensive.py.
...
USE_PROCESSED to False for raw-data plots instead.

ADAPTATION NOTES vs. the Tamil Nadu original:
  - rajasthan_cleaned_physical.csv is ~4 GB, so only the columns the plots
    touch are read (usecols filter). Output is identical.
  - Map centre is taken from the data rather than a hard-coded Tamil Nadu
    centroid.
  - B4's second monsoon band is relabelled "retreat / post-monsoon
    (Oct-Dec)" — Rajasthan has no NE monsoon.

HOW TO RUN
----------
  pip install folium branca matplotlib seaborn
  python 05g_plots_comprehensive.py

OUTPUT
------
  PLOTSV2/comprehensive/
    maps/           — standalone Folium HTML
...
USE_PROCESSED = True   # False -> plots raw climate_rajasthan_points.csv instead

PLOT_DIR = BASE_DIR / "PLOTSV2" / "comprehensive"
for sub in ["maps", "timeseries", "statistics", "solar_resource"]:
    (PLOT_DIR / sub).mkdir(parents=True, exist_ok=True)

DATA_FILE = (PREPROCESSED_DIR / "rajasthan_cleaned_physical.csv") if USE_PROCESSED else COMBINED_POINTS_FILE
...
# Only the columns the plots below touch (keeps the ~4 GB CSV read sane).
WANTED_COLS = {
    "point_id", "lat", "lon", "population", "date", "event", "season", "year", "month",
...
print("=" * 68)
print("  ERA5 Rajasthan — Comprehensive Visualization Batch")
...
print("\nLoading data (columns of interest only) ...")
df = pd.read_csv(DATA_FILE, usecols=lambda c: c in WANTED_COLS, parse_dates=["date"])
...
RJ_CENTER = [float(point_summary["lat"].mean()), float(point_summary["lon"].mean())]
...
m0 = folium.Map(location=RJ_CENTER, zoom_start=7, tiles="CartoDB positron")
...
m1 = folium.Map(location=RJ_CENTER, zoom_start=7, tiles="CartoDB positron")
...
m2 = folium.Map(location=RJ_CENTER, zoom_start=7, tiles="CartoDB dark_matter")
...
ax.plot(state_daily["date"], state_daily["era5_GHI"].rolling(7, min_periods=1).mean(),
        color="#f9c74f", linewidth=2.6, label="Rajasthan mean (7-day rolling)")
...
ax.axvspan(10, 12, alpha=0.08, color="#f3722c", label="retreat / post-monsoon (Oct-Dec)")
