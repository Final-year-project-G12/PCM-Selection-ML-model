"""
09_recommendation_cards.py   (Rajasthan)
=============================================================================
...
NAMING NOTE: RENUMBERED 2026-09-08 from 10_recommendation_cards_rajasthan.py
to 09_recommendation_cards.py, matching Tamil Nadu's convention exactly
(both states: Phase 7 = 10_physics_validation.py, Phase 8 =
09_recommendation_cards.py). The number is NOT the run order here — this
Phase 8 script runs LAST, AFTER 10_physics_validation.py, because it
aggregates that script's output. run_all_rajasthan.py sequences it
accordingly (10 then 09), exactly as run_all_tamilnadu.py does.

PURPOSE: pure aggregation. Pulls cluster_profile_cards_rajasthan.md,
mcdm_full_rankings.csv, and physics_validation_rajasthan.csv into
...
anything:
  1. Provenance-fingerprint stamp check (provenance_lib.py, the same
     ...
     kappa_calibrated.csv, mcdm_full_rankings.csv, and physics_
  ...
  2. An explicit medoid-per-cluster_id cross-check: this script recomputes
     ...
     and 10_physics_validation.py already use) and cross-checks
     it against cluster_profile_cards_rajasthan.md's stated medoid AND
     physics_validation_rajasthan.csv's medoid_point column.
     mcdm_full_rankings.csv carries no medoid column at all (Phase 6
...
INPUTS:
  ...
  data/processed/feasibility_survivors_by_cluster.csv      (Phase 5, primary)
  ...
  08_mcdm_ranking.py (imported as a module, NOT re-run as a
...
SURVIVORS_PRIMARY_FILE = PROCESSED_DIR / "feasibility_survivors_by_cluster.csv"
SURVIVORS_CALIBRATED_FILE = PROCESSED_DIR / "feasibility_survivors_by_cluster_kappa_calibrated.csv"
...
# ═══════════════════════════════════════════════════════════
# DYNAMIC IMPORT — 08_mcdm_ranking.py's filename starts with a
...
def verify_cross_phase_consistency():
    ...
    print(f"  (mcdm_full_rankings.csv carries no medoid column — its cluster-identity "
...
# (feasibility_survivors_by_cluster.csv's own per-row constraint verdicts,
...
def compute_criterion_contributions(mcdm_mod, cand_df, weights, tm_target, l_required):
    ...
    """
    matrix = mcdm_mod.build_criteria_matrix(cand_df, tm_target, l_required)
...
def main():
    ...
    for prof in profiles.itertuples():
        ...
        matrix_for_entropy = mcdm_mod.build_criteria_matrix(cand_df, tm_target, l_required)
        ent_w = mcdm_mod.entropy_weights(matrix_for_entropy)
        blend_w = mcdm_mod.blended_weights(ent_w, cluster_prior_w)
        contributions = compute_criterion_contributions(mcdm_mod, cand_df, blend_w, tm_target, l_required)
    ...
    if any_provisional:
        lines.append("**PROVISIONAL PENDING DATABASE EXPANSION** — every cluster's Top-3 below rests "
                      ...
                      "target; see `mcdm_full_rankings.csv`'s `pcm_database_status` column). Do "
