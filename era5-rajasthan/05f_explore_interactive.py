"""
05f_explore_interactive.py
===========================
ERA5 Rajasthan — Interactive Explorer (Streamlit). The 1:1 port of the
Tamil Nadu pipeline's 05c_explore_interactive.py.

One row per (point_id, date, event) with event in {sunrise, noon, sunset}
— so every time-series chart plots THREE colored traces (one per event)
against date, rather than one continuous hourly line.
...
2. Run:
   streamlit run 05f_explore_interactive.py

3. Browser opens at http://localhost:8501

VIEWS
-----
1 Raw Time Series          — 02_combine output, per sun-event, out-of-bound points flagged.
2 Processed Time Series    — 04_preprocess output (imputed, outlier-filtered, physically validated).
3 Raw vs Processed         — both overlaid for one location/property/event.
4 Location Map             — all points, colored by the selected property's noon mean.

DATA REQUIRED
-------------
  data/processed/climate_rajasthan_points.csv          (02's output — raw)
  data/preprocessed/rajasthan_cleaned_physical.csv     (04's output — processed)

Transient map HTML is written to PLOTSV2/interactive_explorer/.
"""
...
from config import COMBINED_POINTS_FILE, PREPROCESSED_DIR, BASE_DIR

# ═══════════════════════════════════════════════════════════
# PAGE CONFIG — must be the very first Streamlit call
# ═══════════════════════════════════════════════════════════
st.set_page_config(
    page_title="ERA5 Rajasthan — Interactive Explorer",
    page_icon="🌞",
    layout="wide",
...
PROCESSED_FILE = PREPROCESSED_DIR / "rajasthan_cleaned_physical.csv"
OUT_DIR = BASE_DIR / "PLOTSV2" / "interactive_explorer"
...
if not os.path.exists(PROCESSED_FILE):
    st.warning(
        ...
        "Run `04_preprocess_rajasthan.py` first. Raw-only views below still work."
    )

EVENT_ORDER = ["sunrise", "noon", "sunset"]
EVENT_COLORS = {"sunrise": "#f9c74f", "noon": "#4cc9f0", "sunset": "#f3722c"}

# Physical bounds — same dict as 04_preprocess_rajasthan.py, used only to
...
MAP_CENTER = [float(df_raw["lat"].mean()), float(df_raw["lon"].mean())]
...
# ═══════════════════════════════════════════════════════════
st.sidebar.markdown(
    "<h2 style='color:#4cc9f0;margin-bottom:4px'>🌞 ERA5 Rajasthan</h2>"
    f"<p style='color:#8b949e;font-size:12px;margin-top:0'>"
    f"{N_POINTS} population-weighted points · 3 sun-events/day · 2016-2025</p>",
...
# ═══════════════════════════════════════════════════════════
    ...
    if df_proc is None:
        st.error("Processed file not loaded — run `04_preprocess_rajasthan.py` first.")
    else:
...
# ═══════════════════════════════════════════════════════════
    ...
    st.markdown(
        f"<h3 style='color:#06d6a0;margin-bottom:4px'>All {N_POINTS} Points — colored by {prop} (noon mean)</h3>"
        f"<p style='color:#8b949e;font-size:12px;margin-top:0'>"
    ...
    if prop not in source_df.columns:
        ...
        def build_map(summary_df, prop_name, highlight_id, center):
        ...
        map_html = build_map(summary, prop, location, MAP_CENTER)
