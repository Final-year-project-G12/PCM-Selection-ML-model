...
CONSTRAINTS (1-5 pre-existing design, kept as specified; 6-8 new this run):
  ...
  3. Latent heat floor : L >= kappa * L_required, kappa=0.7 FIXED (kept as
                         specified). L_required is now on the combined
                         sensible+latent basis (~305-320 kJ/kg, OPTION A in
                         04_climate_signature_rajasthan.py), so the 0.7x
                         floor is ~215-225 kJ/kg — no longer a wipeout. See
                         "READ THIS FIRST" for current survivor counts.
...
READ THIS FIRST — data facts that determine what this run can actually
show, verified against the real files before writing this script (not
assumed), and re-verified 2026-09-07 after the PCM database was expanded:

  A. THE PCM DATABASE IS NOW 62 ROWS, NOT ~25. 55 manufacturer rows
     (PCM_data/data/PCM_Properties_cleaned_mice_pmm_detailed.csv, MICE+RF+
     PMM-completed) + 7 literature rows (Singh2025 Table 2, reused verbatim
     from the Tamil-Nadu-pipeline reference script that already sourced
     them — see literature_rows() below) = 62 total. This is the same
     database the Tamil Nadu pipeline builds (era5-tamilnadu/
     06_build_pcm_database.py). The earlier state (18 manufacturer + 7
     literature = ~25) and the "40-60 row target never completed" note are
     obsolete — that target is now met. Every count in this script is read
     dynamically (len(manuf), len(pcm_db)); nothing hardcodes a DB size.
  B. NO ENCAPSULATION COLUMN EXISTS ANYWHERE, and ZERO rows in the 62-
     candidate database are salt-hydrate- or Inorganic-typed. Every
     manufacturer row is now Organic (pcm_type values like "Organic
     (RT-line)", "Organic n-alkane", "Organic fatty acid", ...), and all 7
     literature rows are Organic too. The single inorganic row that used
     to be here (savE(R) HS36, Tm=35C) is gone from the expanded database.
     ...
     WILL NOT exclude anything in this run — is_salt_hydrate() matches
     nothing, so every candidate gets c7 = "not_applicable".
  ...
  Constraint 3 is NO LONGER a wipeout. 04_climate_signature_rajasthan.py's
  OPTION A correction (2026-08-31) put L_required on the combined
  sensible+latent basis — SHARE_PCM=0.5 of Q_night — so the current
  cluster_profiles_rajasthan.csv carries L_required ~= 305-320 kJ/kg, not
  the ~600-650 kJ/kg all-latent ceiling the earlier version of this note
  assumed. At kappa=0.7 the floor is ~215-225 kJ/kg, which a good number
  of the 62 candidates clear (manufacturer latent heat spans ~128-260
  kJ/kg; literature 170-252). The most recent primary run produced 4 / 7
  / 5 survivors for clusters 0 / 1 / 2 — clusters 1 and 2 already meet the
  5-survivor target at fixed kappa=0.7, and only cluster 0 falls one
  short. The kappa-calibration companion pass below is therefore now a
  sensitivity check on the latent-heat threshold, not a rescue of an
  all-zero result. 04's "kappa may need per-cluster calibration" note
  (correction #4) still stands as a documented option, but this run no
  longer reproduces a 0-survivor baseline.
...
OUTPUTS (canonical Tamil-Nadu naming — see OUT_FILE below):
  ...
  kappa=0.7 result (currently 4 / 7 / 5 survivors per cluster) is the
  primary result; the calibrated companion shows how the survivor set
  widens as kappa steps down, as a sensitivity check on the latent-heat
  threshold. (On the earlier ~600-650 kJ/kg all-latent L_required this
  fixed-kappa run returned 0 survivors and the pairing was a
  diagnosis-vs-fix demonstration; after 04's OPTION A correction that
  baseline is no longer zero, but reporting the pair still shows how
  sensitive the survivor set is to the kappa choice.)
...
# FIXED kappa=0.7, kept exactly as specified ("keep all of that"). With
# L_required now on the combined sensible+latent basis (04's OPTION A,
# ~305-320 kJ/kg) the 0.7x floor is ~215-225 kJ/kg — see module
# docstring's "READ THIS FIRST" for current per-cluster survivor counts.
...
def is_salt_hydrate(row):
    ...
    for when real salt-hydrate rows / the Assam pipeline need it. Uses a
    substring check on pcm_type rather than an exact "== 'Inorganic'"
    match, because pcm_type is now a descriptive string ("Organic
    (RT-line)", etc.) — an exact match would silently never fire even for
    a future row typed "Inorganic salt hydrate"."""
    fam = str(row["family"]).lower()
    return "inorganic" in str(row["pcm_type"]).lower() or "hydrate" in fam
...
def main():
    ...
    for prof in profiles.itertuples():
        ...
        print(f"\n  --- Cluster {cid} "
              ...
              f"L_required={l_required:.0f} kJ/kg [combined S+L basis, 04 OPTION A], "
        ...
        if n_final_survive < MIN_SURVIVORS_TARGET:
            print(f"    [WARNING] Cluster {cid} still has only {n_final_survive} survivor(s) "
                  ...
                  f"is the likeliest remaining cause, not the melting window this "
                  f"relaxation loop actually widens — check the c3 exclusion count above.")
    ...
    for prof in profiles.itertuples():
        ...
        # kappa is admitted first (needs the smallest drop from 0.7). The
        # "rescuable_by_kappa" set is every candidate that passes c1/c2/c6/
        # c7/c8. Since 04's OPTION A correction, some of these already clear
        # c3 at the fixed kappa=0.7 (breakeven_kappa >= 0.7) and show up as
        # survivors in the primary run; the rest are admitted only once
        # kappa drops to AT OR BELOW their own breakeven_kappa. (On the old
        # ~600-650 kJ/kg L_required every rescuable candidate failed c3 at
        # 0.7, which is why the primary run then showed 0 survivors.)
