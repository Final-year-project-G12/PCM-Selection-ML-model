...
=============================
Interactive (Folium + Plotly) explorer for 05_cluster_rajasthan.py's
Level-A output — the 1:1 port of the Tamil Nadu pipeline's
05b_cluster_interactive.py. Cluster map with hoverable membership
probabilities (the soft GMM assignment a static PNG throws away), plus an
interactive cluster-profile comparison and the K-selection curves.

Requires: pip install plotly folium

INPUT  : data/processed/cluster_assignments_rajasthan_levelA.csv
         data/processed/cluster_profiles_rajasthan.csv
         data/processed/bic_selection_rajasthan.csv
OUTPUT : PLOTSV2/clustering_interactive/*.html

HOW TO RUN:
  python 05e_cluster_interactive.py
"""
...
OUT_DIR = BASE_DIR / "PLOTSV2" / "clustering_interactive"
OUT_DIR.mkdir(parents=True, exist_ok=True)

ASSIGN_FILE = PROCESSED_DIR / "cluster_assignments_rajasthan_levelA.csv"
PROFILE_FILE = PROCESSED_DIR / "cluster_profiles_rajasthan.csv"
BIC_FILE = PROCESSED_DIR / "bic_selection_rajasthan.csv"

PALETTE = px.colors.qualitative.Set1

print("=" * 68)
print("  Interactive Cluster Explorer — Rajasthan (Level A)")
print(f"  Output : {OUT_DIR}/")
print("=" * 68)

for f in (ASSIGN_FILE, PROFILE_FILE):
    if not f.exists():
        raise FileNotFoundError(f"{f} not found — run 05_cluster_rajasthan.py first.")
...
# ═══════════════════════════════════════════════════════════
print("\n[B] Cluster profile comparison (Plotly grouped bar) ...")

# Tamil Nadu names -> Rajasthan signature/profile names:
#   DTR -> DTR_true ; RH_mean -> RH_sunrise_mean ; HSI -> HSI_sunrise
compare_cols = [c for c in ["GHI_daily_kWh", "Ta_mean", "DTR_true", "HDD18", "CDD24",
                             "cloudy_frac", "RH_sunrise_mean", "HSI_sunrise",
...
pop_col = next((c for c in ("total_population", "total_population_covered")
                if c in profiles.columns), None)
if pop_col:
    fig = px.pie(profiles, names="cluster_id", values=pop_col,
