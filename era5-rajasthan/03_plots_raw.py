...
03_plots_raw.py
=================
RAW DATA QA — run this BEFORE any cleaning (04_preprocess_rajasthan.py),
directly on 02_combine_rajasthan.py's output. Purpose: catch pipeline
...
Direct port of the Tamil Nadu pipeline's 03_plots_raw.py — same six
checks (A-F), same schema (population-weighted points x 3 events/day
x 10 years, both ERA5 and NASA POWER columns per row). Only the region
name, the input/preprocess script names, and the output directory
(PLOTSV2/raw/ instead of data/plots/raw/) differ.

WHAT EACH PLOT CHECKS (mapped to Phase 2 acceptance checks in the plan doc)
-----------------------------------------------------------------------
  A. Point map              — sampling design sanity: are the points
                               actually covering Rajasthan, weighted by
  ...
  E. Seasonal boxplots      — GHI/T_amb by season, sanity check against
                               known Rajasthan climatology (hot dry
                               Apr-Jun, SW monsoon Jul-Sep, mild winter).
...
OUTPUT: PLOTSV2/raw/*.png  (all read-only diagnostics — nothing here
writes back to climate_rajasthan_points.csv)
"""
...
from config import COMBINED_POINTS_FILE, BASE_DIR

RAW_PLOT_DIR = BASE_DIR / "PLOTSV2" / "raw"
...
print("=" * 68)
print("  Raw Data QA Plots — Rajasthan Population Points")
print(f"  Input  : {COMBINED_POINTS_FILE}")
print(f"  Output : {RAW_PLOT_DIR}/")
print("=" * 68)

print("\nLoading data (this is a large CSV — may take a minute) ...")
...
print("  -> Non-zero MBE here is exactly what 04_preprocess_rajasthan.py's "
...
print("  in one year, resolve that BEFORE running 04_preprocess_rajasthan.py.")
print("=" * 68)
