...
OUTPUTS:
  ...
  data/processed/cluster_assignments_rajasthan_levelB.csv     (point_id,
      season, seasonal cluster label + probabilities)
...
EXTERNAL VALIDATION: stubbed, not fabricated. No local Koppen-Geiger or
NBC/ECBC lookup exists in this project as of this script — see the
EXTERNAL VALIDATION section below for exactly what's needed to wire each
one in, rather than inventing class labels to compute a number against.
...
def bootstrap_ari_stability(X, k, n_boot=N_BOOTSTRAP, random_state=RANDOM_STATE):
    ...
    for b in range(n_boot):
        ...
        try:
            ...
            continue
    return float(np.mean(aris)) if aris else float("nan"), base_labels
...
def fit_k_range(X, k_values, label):
    ...
    for k in k_values:
        ...
        boot_ari, _ = bootstrap_ari_stability(X, k)
        ...
        rows.append({
            ...
            "bootstrap_ari_mean": boot_ari, "kmeans_silhouette": km_sil,
...
hard_labels_a = gmm_final_a.fit_predict(X_a)
soft_probs_a = gmm_final_a.predict_proba(X_a)
...
print("""
  Koppen-Geiger classification: NOT WIRED IN.
  TODO to enable: source the Beck et al. 2018 1-km Koppen-Geiger raster
  ("Present and future Koppen-Geiger climate classification maps at 1-km
  resolution," Scientific Data 5, 180214, doi:10.1038/sdata.2018.214) or
  an equivalent already-published lookup, save it under
  data/raw/koppen/, then point-sample it at each point_id's (lat, lon)
  (e.g. via rasterio, since geopandas/rasterio are already project
  dependencies for 00a_build_population_grid.py) to get one Koppen class
  per point. Feed the resulting labels and this script's hard_labels_a
  into sklearn.metrics.adjusted_rand_score / normalized_mutual_info_score.
  Stubbed here rather than fabricated — no local file exists in this
  project tree as of this run.
...
for cid in sorted(sig["cluster_id"].unique()):
    ...
    row = {"cluster_id": int(cid), "n_points": len(g), "total_population": g["population"].sum()}
...
lines = [f"# {STATE_NAME.title()} — Level A Cluster Profile Cards\n",
         ...
         f"{reason_a}. This is a single-state run — see the printed "
         f"EXTERNAL VALIDATION section for what's not yet wired in.\n"]
...
for cid in sorted(sig["cluster_id"].unique()):
    ...
    for col in SIGNATURE_DISPLAY:
        wm = weighted_mean(g, col)
        sd = float(g[col].std())
        lines.append(f"| {col} | {wm:.3f} | {sd:.3f} |")

    cluster_means = {c: weighted_mean(g, c) for c in SIGNATURE_DISPLAY}
    ...
    l_req = weighted_mean(g, "L_required_kJ_per_kg") if "L_required_kJ_per_kg" in g.columns else float("nan")
...
print(f"  Outputs: {BIC_TABLE_FILE.name}, {ASSIGN_A_FILE.name}, {ASSIGN_B_FILE.name}, "
      f"{MAP_FILE.name}, {CARDS_FILE.name}")
