...
============================
Interactive (Plotly + Folium) raw-data QA for Rajasthan — the 1:1 port of
the Tamil Nadu pipeline's 03b_interactive_raw_qa.py (same six checks A-F),
reading climate_rajasthan_points.csv (02_combine_rajasthan.py's output) as
zoomable/hoverable HTML instead of static PNGs. Nothing here writes back to
the data; purely a read-only QA pass.

This sits alongside Rajasthan's own 03c_plots_raw_rajasthan.py (which
already covers A/B/C/D/E in Plotly). It is kept as the exact structural
twin of the Tamil Nadu script so the two states' raw-QA output lines up
file-for-file when reviewing cross-region.

Requires: pip install plotly folium branca

INPUT  : data/processed/climate_rajasthan_points.csv
OUTPUT : PLOTSV2/raw_interactive/*.html

HOW TO RUN:
  python 03d_interactive_raw_qa.py
Then open the .html files in a browser (plotly.js is pulled from CDN, so
you need internet the first time each file is opened; the folium map embeds
its own tiles/JS and works offline once loaded).
"""
...
OUT_DIR = BASE_DIR / "PLOTSV2" / "raw_interactive"
...
print("=" * 68)
print("  Interactive Raw Data QA — Rajasthan Population Points")
