...
==========================
POST-PREPROCESSING QA (static PNG) — the 1:1 port of the Tamil Nadu
pipeline's 04c_postprocess_plots.py. Run AFTER 04_preprocess_rajasthan.py,
on its output (rajasthan_cleaned_physical.csv). 03c_plots_raw_rajasthan.py
checked the RAW data before cleaning; this script checks the CLEANED data
after cleaning, so you can see exactly what the 13 preprocessing steps
actually did before trusting them for Phase 3.

WHAT EACH PLOT CHECKS
-----------------------------------------------------------------------
  A. Missing-data heatmap (post)   — should be essentially all-zero.
  B. Distribution sanity            — histograms of the core physical
                                      columns after outlier-NaN'ing +
                                      imputation. Watch for imputation
                                      spikes at a point/zone/global median.
  C. Physical-bounds + Hampel impact — reads qc_report.txt and plots how
                                      many values step 2 (physical bounds)
                                      and step 3 (Hampel/MAD) each flagged.
  D. Lag-feature sanity             — GHI vs GHI_lag7d scatter; correlation
                                      should be positive and clearly
                                      structured, not noise.
  E. One-point time series          — cleaned noon GHI over one year for
                                      one point, 7d/30d rolling mean
                                      overlaid, to eyeball that cleaning
                                      didn't flatten the seasonal shape.
  F. Correlation heatmap (post)     — cleaned + engineered columns.

ADAPTATION NOTE: rajasthan_cleaned_physical.csv is ~4 GB, so this reads
only the columns the six checks touch (usecols filter) rather than the
whole frame the way the Tamil Nadu script does. Output is identical.

INPUT  : data/preprocessed/rajasthan_cleaned_physical.csv
OUTPUT : PLOTSV2/post_preprocess/*.png

HOW TO RUN:
  python 04d_postprocess_plots.py
"""
...
from config import PREPROCESSED_DIR, BASE_DIR

POST_PLOT_DIR = BASE_DIR / "PLOTSV2" / "post_preprocess"
...
# Only these columns are needed by checks A-F (see below) — keeps the
# ~4 GB CSV read to something a laptop can hold.
WANTED_COLS = {
    "point_id", "date", "event", "year", "is_daytime",
    "era5_T_amb", "era5_GHI", "era5_RHum", "era5_cloud_cover", "era5_precipitation",
    "era5_CSI", "era5_W_spd", "power_ALLSKY_SFC_SW_DWN", "power_T2M",
    "era5_GHI_lag7d", "era5_GHI_roll7d_mean", "era5_GHI_roll30d_mean",
    "era5_DNI", "era5_DHI", "era5_cloud_opacity", "era5_T_depression",
    "solar_hour_angle", "era5_GHI_delta1d", "era5_T_amb_delta1d",
}

print("=" * 68)
print("  Post-Preprocessing QA Plots — Rajasthan")
print(f"  Input  : {PHYSICAL_FILE}")
print(f"  Output : {POST_PLOT_DIR}/")
print("=" * 68)

if not PHYSICAL_FILE.exists():
    raise FileNotFoundError(f"{PHYSICAL_FILE} not found — run 04_preprocess_rajasthan.py first.")

print("\nLoading cleaned data (columns of interest only) ...")
df = pd.read_csv(PHYSICAL_FILE, usecols=lambda c: c in WANTED_COLS, parse_dates=["date"])
...
if QC_REPORT.exists():
    ...
    # Tamil Nadu wording: "<col>: <n> out-of-range values"
    # Rajasthan wording : "<col>: <n> values forced to 0 (...)" / "<n> values NaN'd"
    bound_hits = re.findall(
        r"^\s*(\S+): ([\d,]+) (?:out-of-range values|values forced to 0|values NaN'd)",
        text, re.MULTILINE)
    hampel_hits = re.findall(
        r"^\s*(\S+): ([\d,]+) points flagged as Hampel outliers", text, re.MULTILINE)
    ...
    if rows:
        ...
        ax.set_title("Values flagged NaN/0 by physical-bounds vs. Hampel filter (steps 2 & 3)")
        ax.set_xlabel("count")
        plt.tight_layout()
    ...
    print(f"  [SKIP] {QC_REPORT} not found — rerun 04_preprocess_rajasthan.py to regenerate it.")
...
print("  go back and check 04_preprocess_rajasthan.py's qc_report.txt")
print("  BEFORE running the climate-signature step.")
print("=" * 68)
