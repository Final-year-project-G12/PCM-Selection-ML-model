...
data (climate_rajasthan_points.csv), same six checks (A-F), but as
...
# This file lives two levels down in PLOTSV2/interactive_plots, so the pipeline
# root goes on the path before importing config (same shim as 03e/00e).
...
from config import COMBINED_POINTS_FILE

# Outputs go next to this pipeline's plot scripts, in PLOTSV2/raw_interactive/,
# not under data/plots/.
OUT_DIR = Path(__file__).resolve().parents[1] / "raw_interactive"
