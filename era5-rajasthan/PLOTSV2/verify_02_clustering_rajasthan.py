...
Output: PLOTSV2/verify_clustering/
Run AFTER: 05_cluster_rajasthan.py

Port of tamilnadu_pipeline/plots/verify_02_clustering_tamilnadu.py and
era5-uttarakhand/verify_02_clustering.py - same 6 plots, same style.

Uses the LEVEL A assignments (annual GMM, k=3, one row per grid point). That
is the level the whole PCM chain downstream runs on - cluster_profiles,
feasibility, MCDM and the 13 objective-1 plots are all Level A - so verifying
Level B here (as plotting/verify_02_clustering_rajasthan.py does) would be
checking a partition nothing else in this plot set uses. Set LEVEL = "B" to
verify the seasonal k=8 partition instead.
"""
...
CLU_CSV  = os.path.join(BASE,"data","processed",f"cluster_assignments_rajasthan_level{LEVEL}.csv")
OUT      = os.path.join(os.path.dirname(os.path.abspath(__file__)),"verify_clustering")
os.makedirs(OUT, exist_ok=True)

print("Loading data...")
try:
    sig=pd.read_csv(SIG_CSV); clu=pd.read_csv(CLU_CSV)
    print(f"  Signature: {sig.shape}  Clusters (Level {LEVEL}): {clu.shape}")
...
# Get numeric features (skip non-numeric, metadata and already-standardised
# _z / PC columns - keeping those would double-count the same signal)
DROP={"point_id","cluster_id","lat","lon","c_lat","c_lon","population","id","weight","elevation_m"}
fc=[c for c in mg.columns
    if c not in DROP and not c.endswith("_z") and not c.startswith("PC")
    and pd.api.types.is_numeric_dtype(mg[c])]
...
plt.suptitle(f"Rajasthan - Cluster-Count Diagnostics (Level {LEVEL})",fontsize=14)
...
ax.set(xlabel="Silhouette Coeff",ylabel="Cluster",title=f"Silhouette - Rajasthan clusters (Level {LEVEL}, k={k})")
...
ax.set(xlabel=f"PC1 ({pca.explained_variance_ratio_[0]:.1%})",ylabel=f"PC2 ({pca.explained_variance_ratio_[1]:.1%})",title=f"PCA Projection - Rajasthan clusters (Level {LEVEL}, k={k})")
...
if not cd.empty:
    ...
    ax.set(xlabel="Longitude",ylabel="Latitude",title=f"Geographic Distribution - Rajasthan clusters (Level {LEVEL}, k={k})")
    plt.colorbar(sc,ax=ax,label="Cluster"); ax.grid(alpha=0.3); plt.tight_layout()
...
cm=mg.copy()
KEY_FEATS=["Ta_mean","GHI_daily_kWh","kt_daily_mean","RH_sunrise_mean","DTR_true","Tm_target_capped_C"]
kf=[c for c in KEY_FEATS if c in fc] or fc[:6]
fig,axes=plt.subplots(2,3,figsize=(15,10)); axes=axes.flatten()
for i,feat in enumerate(kf[:6]):
    sns.boxplot(data=cm,x="cluster_id",y=feat,hue="cluster_id",palette="Set2",legend=False,ax=axes[i])
    axes[i].set_title(f"{feat} by Cluster"); axes[i].grid(alpha=0.3,axis="y")
for j in range(len(kf[:6]),len(axes)): axes[j].axis("off")
plt.suptitle(f"Rajasthan - Cluster Profiles (Level {LEVEL})",fontsize=14,y=1.0); plt.tight_layout()
...
ax.set(xlabel="Cluster ID",ylabel="Grid points",title=f"Cluster Size Distribution - Rajasthan (Level {LEVEL}, k={k})")
...
print("\n" + "="*60)
print(f"CLUSTERING VALIDATION SUMMARY (Rajasthan, Level {LEVEL})")
print("="*60)
