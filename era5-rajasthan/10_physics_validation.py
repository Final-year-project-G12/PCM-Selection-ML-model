"""
10_physics_validation.py   (Rajasthan)
=============================================================================
PHASE 7 — PHYSICS-BASED VALIDATION OF THE MCDM RANKING, RAJASTHAN

RENUMBERED 2026-09-08 from 09_physics_validation_rajasthan.py to match Tamil
Nadu's convention exactly: Phase 7 = 10_physics_validation.py, Phase 8 =
09_recommendation_cards.py in both state folders (cards run LAST, after
physics, despite the lower number).

Phase 6 (08_mcdm_ranking.py) produced a consensus PCM ranking per
...
mcdm_full_rankings.csv's 5+8+7=20 rows — a few seconds each, so a
...
def write_summary_txt(spearman_df, interpretation_paragraphs, calib_df, extra, cluster_id_mismatch):
    ...
    lines.append("[DATA PROVENANCE] Cross-file fingerprint check PASSED — "
                 "feasibility_survivors_by_cluster_kappa_calibrated.csv (Phase 5) and "
                 "mcdm_full_rankings.csv (Phase 6) were both confirmed built from the SAME "
