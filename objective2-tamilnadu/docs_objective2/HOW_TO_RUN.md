...
## 8. Plots — justification figures for Phases 2-7
...
plots). Needs `plotly` and `kaleido` (installed already in this project —
`pip install kaleido` is the only new package Phases 1-7 didn't already
need). Runtime: ~1-2 minutes. Writes 15 figures, each as:
