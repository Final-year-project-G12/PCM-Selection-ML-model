...
03_plots_raw.py
=================
RAW DATA QA — run this BEFORE any cleaning (04_preprocess_tamilnadu.py),
directly on 02_combine_tamilnadu.py's output. Purpose: catch pipeline
...
This replaces the old 05_plot_tamilnadu.py (which assumed a continuous
hourly series across 260+ named cities). This version is built for the
new schema: 133 population-weighted points x 3 events/day (sunrise/noon/
sunset) x 10 years, with both ERA5 and NASA POWER columns per row.

WHAT EACH PLOT CHECKS (mapped to Phase 2 acceptance checks in the plan doc)
-----------------------------------------------------------------------
  A. Point map              — sampling design sanity: are the 133 points
                               actually covering Tamil Nadu, weighted by
  ...
  E. Seasonal boxplots      — GHI/T_amb by season, sanity check against
                               known Tamil Nadu climatology (hot dry
...
writes back to climate_tamilnadu_points.csv)
"""
...
print("=" * 68)
print("  Raw Data QA Plots — Tamil Nadu Population Points")
...
ax.set_title(f"Tamil Nadu — {len(point_meta)} Population-Weighted Sampling Points")
...
print("  -> Non-zero MBE here is exactly what 04_preprocess_tamilnadu.py's "
...
print("  in one year, resolve that BEFORE running 04_preprocess_tamilnadu.py.")
print("=" * 68)
