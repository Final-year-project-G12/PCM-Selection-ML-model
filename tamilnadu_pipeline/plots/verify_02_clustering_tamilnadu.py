"""
Verification Script 2: Clustering Validation - Tamil Nadu
Output: data/plots/verify_clustering/
Run AFTER: 05_cluster_tamilnadu.py
"""
...
CLU_CSV  = os.path.join(BASE,"data","processed","clustering","cluster_assignments_tamilnadu.csv")
...
cg=clu[["point_id","cluster_id","lat","lon"]].rename(columns={"lat":"c_lat","lon":"c_lon"})
mg=sig.merge(cg,on="point_id",how="inner")
...
fc=[c for c in mg.columns if c not in {"point_id","cluster_id","lat","lon","c_lat","c_lon","population"} and pd.api.types.is_numeric_dtype(mg[c])]
...
ax.set(xlabel="Silhouette Coeff",ylabel="Cluster",title=f"Silhouette - Tamil Nadu clusters (k={k})")
...
ax.set(xlabel=f"PC1 ({pca.explained_variance_ratio_[0]:.1%})",ylabel=f"PC2 ({pca.explained_variance_ratio_[1]:.1%})",title=f"PCA Projection - Tamil Nadu clusters (k={k})")
...
if not cd.empty:
    ...
    ax.set(xlabel="Longitude",ylabel="Latitude",title=f"Geographic Distribution - Tamil Nadu clusters (k={k})")
    plt.colorbar(sc,ax=ax,label="Cluster"); ax.grid(alpha=0.3); plt.tight_layout()
...
plt.suptitle("Tamil Nadu - Cluster Profiles",fontsize=14,y=1.0); plt.tight_layout()
...
ax.set(xlabel="Cluster ID",ylabel="Samples",title=f"Cluster Size Distribution - Tamil Nadu (k={k})")
...
print("\nCLUSTERING VALIDATION SUMMARY (Tamil Nadu)")
print("="*60)
