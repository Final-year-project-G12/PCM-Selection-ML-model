"""
COMBINE — TAMIL NADU POPULATION POINTS  (ERA5 + NASA POWER cross-check)
=============================================================================
Reads  :
  data/raw/era5/points/era5_TN_points_{year}_{month}_{instant,accum}.nc
  data/raw/nasapower/power_{point_id}_{year}.json
  data/processed/population_grid_points.csv
  data/processed/suntimes.csv
Writes :
  data/processed/climate_tamilnadu_points.csv
...
This replaces the old named-location (TN_LOCATIONS, 260+ cities) and opt-in
full-grid combine logic entirely — matches 02_combine_rajasthan.py exactly
so both regions produce directly comparable schemas for the later
cross-region clustering step (Objective 1).
...
Safe to re-run — overwrites climate_tamilnadu_points.csv with fresh data.
"""
...
def load_all_files():
    ...
    for year in YEARS:
        ...
        accum_ds[year]   = {}
        for month in MONTHS:
            fi = os.path.join(INPUT_DIR, f"era5_TN_points_{year}_{month}_instant.nc")
            fa = os.path.join(INPUT_DIR, f"era5_TN_points_{year}_{month}_accum.nc")
...
if __name__ == "__main__":
    ...
    print("=" * 68)
    print("  ERA5 + NASA POWER COMBINE — TAMIL NADU POPULATION POINTS")
    ...
    instant_count = sum(
        ...
        if os.path.exists(os.path.join(INPUT_DIR, f"era5_TN_points_{y}_{m}_instant.nc")))
    accum_count = sum(
        1 for y in YEARS for m in MONTHS
        if os.path.exists(os.path.join(INPUT_DIR, f"era5_TN_points_{y}_{m}_accum.nc")))
    expected = len(YEARS) * len(MONTHS)
    power_count = sum(
    ...
    if instant_count == 0 and accum_count == 0:
        print("\n  ⚠️  No ERA5 files found — run 01_download_era5_tamilnadu.py first.")
    if power_count == 0:
    ...
    print(f"  Output           : {COMBINED_POINTS_FILE}")
    print("=" * 68)
    print("\nNext: load climate_tamilnadu_points.csv for training / cross-checking, "
          "or run the cross-region clustering step (Objective 1) alongside "
          "climate_rajasthan_points.csv and the other two regions.")
