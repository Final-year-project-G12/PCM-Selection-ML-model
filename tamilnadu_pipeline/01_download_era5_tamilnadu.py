"""
ERA5 DOWNLOAD — TAMIL NADU POPULATION-WEIGHTED POINTS  (sun-event-aligned, 10-year history)
=============================================================================
...
Tamil Nadu, not the old 260+ named-location list — at UTC hours computed
from suntimes.csv (built by 00b_build_suntimes.py) rather than fixed clock
hours. Identical method to 01_download_era5_rajasthan.py.
...
---------------------------------------------------------
Rather than fixed clock hours, this computes three separate narrow UTC hour
windows — one around sunrise, one around solar noon, one around sunset —
each wide enough to cover the seasonal spread of that event across every
point in population_grid_points.csv, all year, plus a ~1hr margin. These
three windows are unioned into INSTANT_HOURS.

Tamil Nadu's longitude span (~76.8°E–80.3°E) is narrower than Rajasthan's,
so a cross-midnight-UTC sun event is unlikely but not impossible for a
far-western point near winter solstice — the same circular (mod-24) window
logic is used regardless, so this is handled correctly either way without
needing a TN-specific special case.

Two separate API calls per month (ERA5 rule), same as the original pipeline:
  ...
  • accum    — forecast variables (TYPE=FC): solar radiation, precipitation
      ...
      deaccumulate() in 02_combine_tamilnadu.py), every target hour's
      immediate predecessor is also downloaded:
          ACCUM_HOURS = INSTANT_HOURS ∪ {(h - 1) mod 24 for h in INSTANT_HOURS}
      deaccumulate()'s reset_mask = hour.isin([1, 13]) generalizes correctly
      to this wider/non-fixed hour set — see that function's docstring.

      One true edge case: 2016-01-01 has no 2015-12-31 file to supply hour
      23 as hour 0's predecessor. Every other month boundary is bridged
      automatically because 02_combine_tamilnadu.py concatenates every
      month/year into one continuous sorted series per point before calling
      deaccumulate() — only that single first day of the whole 10-year
      dataset is short a predecessor, and pandas' diff() naturally yields
      NaN there rather than a silently wrong value.

HOW TO RUN:
  1. Save your API key in this folder's .cdsapirc (see config.py) or set
     CDSAPI_URL / CDSAPI_KEY environment variables.
  2. Run 00a_build_population_grid.py and 00b_build_suntimes.py first.
  3. python 01_download_era5_tamilnadu.py
...
old full-state-grid download (RAW_GRID_DIR / DOWNLOAD_STATUS_FILE), so
nothing here touches any 222-point uniform-grid NetCDF already downloaded
under the previous fixed-hour scheme.
"""
...
def circular_hour_window(hours_observed, margin=HOUR_MARGIN):
    ...
    `hours_observed`, expanded by `margin` hours on each end. Handles the
    UTC-midnight wraparound case correctly (see module docstring)."""
...
TN_BBOX, N_POINTS = load_points_bbox()
...
def main():
    print("\n" + "═" * 68)
    print("  ERA5 Tamil Nadu — Population Points Download  (sun-event hours, 10 years)")
    print(f"  Started : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Points  : {N_POINTS}  (from {POPULATION_GRID_FILE.name})")
    print(f"  BBox    : N={TN_BBOX[0]:.2f} W={TN_BBOX[1]:.2f} "
          f"S={TN_BBOX[2]:.2f} E={TN_BBOX[3]:.2f}")
    ...
    print("\nNext step: run  01b_download_nasapower.py  (if not already done), "
          "then  00_unzip_accum.py,  then  02_combine_tamilnadu.py")
