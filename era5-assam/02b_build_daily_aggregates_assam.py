...
================================
PHASE 2 REPAIR 1 (Objective 1 Plan v3.0, Section 4.3) — ASSAM

climate_assam_points.csv keeps only 3 rows/day (sunrise, noon, sunset).
...
02_combine_assam.py only ever read 3 of those ~8760 hours per year.
...
HOW TO RUN:
  python 02b_build_daily_aggregates_assam.py
"""
...
OUT_DAILY = PROCESSED_DIR / "daily_aggregates_assam.csv"
OUT_TIER2 = PROCESSED_DIR / "tier2_signature_assam.csv"
...
def daily_from_hourly(hourly):
    ...
    hourly.index = hourly.index.tz_convert("Asia/Kolkata")
    ...
    if {"GHI_daily_kWh", "GHIcs_daily_kWh"}.issubset(out.columns):
        out["kt_daily"] = np.where(out["GHIcs_daily_kWh"] > 0.05,
                                    (out["GHI_daily_kWh"] / out["GHIcs_daily_kWh"]).clip(0, 1.2),
                                    np.nan)
...
def main():
    print("=" * 68)
    print("  PHASE 2 REPAIR 1 — Daily-Integral (Tier 2) Indices — Assam")
    print("=" * 68)
    ...
    print("=" * 68)
    print("\nNext: run 04_preprocess_assam.py (Phase 2) if not already done, "
