...
MODEL (3-phase, adapted from Barqawi2025's ODE structure)
--------------------------------------------------------------------------
...
daily GHI/temperature data.

Each hourly step is solved with BACKWARD EULER (implicit).
...
--------------------------------------------------------------------------
  Tank water mass Mw          150 kg
  Collector-tank coil area Ac  2.5 m^2
  Water-coil HTC hc            1500 W/m^2K
  Collector efficiency eff     0.70
  PCM volume                   0.035 m^3
  PCM-water HTC hp             800 W/m^2K
  PCM surface area Ap          3.5 m^2
  ...
  Ambient temp                 daily sinusoid built from that day's real
                               ...
                               local, trough 05:00 local

CALIBRATION CHECK (plan v3.0 Table 16)
------------------------------------------
Annual solar fraction is expected to land in the 54-84% published range
for SWH systems (Table 16).

OUTPUT INTERPRETATION (plan v3.0 Table 17)
--------------------------------------------------------------------------
  rho > 0.8       : MCDM ranking predicts physical performance (strong)
  0.4 < rho < 0.8 : partial agreement
  rho < 0.4       : MCDM ranking doesn't predict performance
...
DAILY_FILE = PROCESSED_DIR / "daily_aggregates_assam.csv"
ASSIGN_FILE = PROCESSED_DIR / "clustering" / "cluster_assignments_assam.csv"
SCORES_FILE = PROCESSED_DIR / "pcm" / "mcdm_full_scores_assam.csv"
OUT_RESULTS = PROCESSED_DIR / "pcm" / "physics_validation_results_assam.csv"
OUT_SPEARMAN = PROCESSED_DIR / "pcm" / "physics_validation_spearman_assam.csv"
...
def pick_medoid_point(assign_df, cluster_id):
    ...
    if "max_membership_prob" in sub.columns:
        return sub.loc[sub["max_membership_prob"].idxmax(), "point_id"]
    # Fallback if max_membership_prob isn't present
    return sub["point_id"].iloc[0]
...
def build_hourly_drivers(year_df, sun_df, point_id):
    ...
    for _, day in year_df.iterrows():
        ...
        if d in sun_pivot.index and pd.notna(sun_pivot.loc[d].get("sunrise", pd.NaT)) \
            ...
            sunrise_hr, sunset_hr = 5.75, 18.0   # fallback: Assam approx
...
def main():
    print("=" * 68)
    print("  Phase 7 -- Grey-Box Physics Validation -- Assam")
    print("=" * 68)
