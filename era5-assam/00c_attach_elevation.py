"""
ELEVATION ATTACHMENT — ASSAM POPULATION POINTS
=============================================================================
Population points carry no elevation field, so 02_combine_assam.py falls
back to a flat DEFAULT_ALT_M=100 approximation for every point when
calling pvlib for solar geometry / clear-sky irradiance. That's a
reasonable simplification only where terrain varies little across the
study area — it breaks down anywhere relief is large within the sampled
region.

Assam's populated terrain is mostly the flat Brahmaputra valley (~30-100m),
but the sampling envelope also touches the hill districts bordering it
(Karbi Anglong, North Cachar Hills / Dima Hasa, and the foothills near
Meghalaya/Nagaland/Arunachal), which reach several hundred metres. A flat
100m default silently over- or under-states elevation for those points'
solar geometry the same way Rajasthan/Tamil Nadu/Uttarakhand's equivalent
bug did (fixed there via this same script, ported here).
...
envelope 01_download_era5_assam.py uses — and attaches a per-point
...
Output:
  data/raw/era5/invariant/era5_ASM_geopotential.nc   (raw cache)
...
Caveat (same as the other states' version of this script): ERA5's native
grid is ~0.25 deg (~28km), so its orography is a grid-cell MEAN elevation.
A single cell value smooths out real local relief in the hill districts.
This is an accepted, documented limitation, not something this script
tries to fix further — but a cell-mean elevation is still far closer to
the truth than one flat number for the whole state.
"""
...
# Everything below this block is identical to era5-rajasthan/era5-tamilnadu's
# 00c_attach_elevation.py (DEM source: ERA5 time-invariant geopotential;
# extraction: nearest-cell z / g0).
# ═══════════════════════════════════════════════════════════
STATE_NAME = "assam"
STATE_CODE = "ASM"
...
# mirroring era5-tamilnadu/00c_attach_elevation.py.
...
BBOX_PAD_DEG = 0.5   # same padding 01_download_era5_assam.py's load_points_bbox() uses
...
# create it here so this script is self-contained.
RAW_INVARIANT_DIR.mkdir(parents=True, exist_ok=True)


# ═══════════════════════════════════════════════════════════
# BBOX  (same logic as 01_download_era5_assam.py's load_points_bbox)
# ═══════════════════════════════════════════════════════════
...
def open_nc(fpath):
    """Same multi-engine fallback as 02_combine_assam.py's open_nc."""
