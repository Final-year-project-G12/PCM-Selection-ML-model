"""
08_mcdm_ranking.py  -- Assam
==================================================================
PHASE 6 - MULTI-CRITERIA RANKING ENGINE (plan v3.0 Section 9)

Full 4-method MCDM stack + 5,000-draw Monte Carlo stability.
Adapted from the Tamil Nadu pipeline's validated implementation.

THE ONE STEP EVERY PCM-MCDM PAPER GETS WRONG (plan Section 9.2)
-----------------------------------------------------------------
Melting temperature is TARGET-based. Converted to Gaussian fitness
BEFORE entering the decision matrix:

    f_Tm(i) = exp(-(Tm_i - Tm_target)^2 / (2*sigma^2)), sigma=4K

CRITERIA (all benefit after transformation)
-------------------------------------------
  f_Tm                    -- Gaussian Tm fitness (target-based)
  latent_heat_margin_ratio -- L / L_required (climate-relative; avoids
                              constant winner when Tm_target is fixed)
  rho_H_MJ_m3             -- volumetric latent heat (tank size)
  TC_W_mK                 -- thermal conductivity (charge/discharge rate)
  cycles_confidence        -- log-scaled cycling stability

FOUR RANKING METHODS
--------------------
  TOPSIS       -- closeness coefficient C_i in [0,1]
  GRA          -- grey relational grade vs ideal reference
  PROMETHEE II -- net outranking flow phi; V-shape preference function
  VIKOR        -- compromise index Q_i + acceptable-advantage check

CONSENSUS
---------
  Borda count (primary) + Copeland pairwise (cross-check)
  Kendall's W per cluster; flagged if Borda/Copeland disagree on #1

MONTE CARLO (plan Section 9.6)
-------------------------------
  5,000 draws. Per draw: Dirichlet weight perturbation + Gaussian
  property noise (Tm +/-1K, L +/-5%, TC +/-10%, rhoH +/-8%).
  Reports Top-3 inclusion probability per PCM per cluster.

INPUT  : data/processed/pcm/feasibility_survivors_assam.csv
OUTPUT : data/processed/pcm/mcdm_topk_assam.csv
...
SURVIVORS_FILE = PROCESSED_DIR / "pcm" / "feasibility_survivors_assam.csv"
OUT_TOPK       = PROCESSED_DIR / "pcm" / "mcdm_topk_assam.csv"
OUT_FULL       = PROCESSED_DIR / "pcm" / "mcdm_full_scores_assam.csv"
OUT_MC         = PROCESSED_DIR / "pcm" / "monte_carlo_stability_assam.csv"
...
LATENT_CRITERION_NAME = ("latent_heat_margin_ratio"
                         if USE_CLIMATE_RELATIVE_LATENT_HEAT else "latent_heat_kJ_kg")
...
def gaussian_tm_fitness(tm, tm_target, sigma_lower=4.0, sigma_upper=1.5):
    # Asymmetric fitness: diagnosed from Phase 7 physics validation.
    # SF crashes for Tm > target because the collector can't reach it.
...
def borda_and_copeland(rank_series_list):
    ...
    for i, name_i in enumerate(names):
        for jx, name_j in enumerate(names):
            if i == jx:
                continue
            wins   = sum(1 for r in rank_series_list if r[name_i] < r[name_j])
            losses = sum(1 for r in rank_series_list if r[name_i] > r[name_j])
...
# ===================================================================
# MONTE CARLO STABILITY  (plan Section 9.6)
# ===================================================================

def run_monte_carlo(df, tm_target, w_final,
                    n_draws=N_MONTE_CARLO_DRAWS, seed=MC_RANDOM_SEED):
    rng      = np.random.default_rng(seed)
    names    = df["name"].tolist()
    n_cand   = len(names)
    l_req    = (df["L_required_kJ_per_kg"].iloc[0]
                if "L_required_kJ_per_kg" in df.columns else np.nan)

    base_matrix      = minmax_normalize(df, CRITERIA)
    baseline_scores  = topsis(base_matrix, w_final)
    baseline_rank    = pd.Series(baseline_scores, index=names).rank(
    ...
    for _ in range(n_draws):
        ...
        tc_draw  = df["TC_W_mK"].values * (
            1 + rng.normal(0, MC_RELATIVE_STD["TC_W_mK"], n_cand))
        rho_draw = df["rho_H_MJ_m3"].values * (
            1 + rng.normal(0, MC_RELATIVE_STD["rho_H_MJ_m3"], n_cand))

        latent_draw = (l_draw / l_req) if USE_CLIMATE_RELATIVE_LATENT_HEAT else l_draw

        draw_df = pd.DataFrame({
            "f_Tm":               gaussian_tm_fitness(tm_draw, tm_target),
            LATENT_CRITERION_NAME: latent_draw,
        ...
        draw_rank   = pd.Series(draw_scores, index=names).rank(
                          ascending=False, method="min")
...
def rank_cluster(df):
    ...
    df["f_Tm"] = gaussian_tm_fitness(df["Tm_C"], tm_target)

    if USE_CLIMATE_RELATIVE_LATENT_HEAT:
        l_req = df["L_required_kJ_per_kg"].iloc[0]
        df["latent_heat_margin_ratio"] = df["latent_heat_kJ_kg"] / l_req

    df["cycles_confidence_imputed"] = df["cycles_confidence"].isna()
    med = df["cycles_confidence"].median()
    df["cycles_confidence"] = df["cycles_confidence"].fillna(
        med if med == med else 0.5)
    ...
    vq, vs, vr            = vikor(M, w_final)
    df["vikor_Q"] = vq;  df["vikor_S"] = vs;  df["vikor_R"] = vr
    ...
    df["vikor_rank"]     = df["vikor_Q"].rank(ascending=True, method="min").astype(int)

    vikor_valid, vikor_note = vikor_compromise_check(vq, df["name"].values)
    df["vikor_compromise_note"] = vikor_note

    rank_series = [df.set_index("name")[c]
                   for c in ["topsis_rank", "gra_rank", "promethee_rank", "vikor_rank"]]
    ...
    mc = run_monte_carlo(df, tm_target, w_final)
...
def main():
    ...
    print(f"  + {N_MONTE_CARLO_DRAWS}-draw Monte Carlo -- Assam")
    print("=" * 68)

    if not SURVIVORS_FILE.exists():
        print(f"\n  ERROR: {SURVIVORS_FILE} not found.")
        print("  Run 07_feasibility_filter.py first.")
        return
    ...
    for cid, grp in survivors.groupby("cluster_id"):
        ...
        print(f"\n  Cluster {int(cid)}: {len(passed)} survivors -- running 4-method stack ...")
        ...
        top3       = ranked.head(3)
        agree_flag = ("" if ranked["borda_copeland_agree"].iloc[0]
                      else "  [Borda/Copeland DISAGREE on #1]")
        print(f"\n  Cluster {int(cid)}  "
              f"(Tm_target={passed['Tm_target_C'].iloc[0]:.1f}C  "
              f"L_req={passed['L_required_kJ_per_kg'].iloc[0]:.0f} kJ/kg  "
              f"n={len(passed)}  Kendall W={ranked['kendall_w'].iloc[0]:.3f})"
              f"{agree_flag}:")

        for _, row in top3.iterrows():
            mc_row = mc[mc["name"] == row["name"]]
            incl   = mc_row["top3_inclusion_probability"].iloc[0] if len(mc_row) else float("nan")
            print(f"    #{row['consensus_rank']}  {row['name']:38s}  "
                  f"Tm={row['Tm_C']:.1f}C  "
        ...
        print(f"    VIKOR: {ranked['vikor_compromise_note'].iloc[0]}")
    ...
    print("\n" + "=" * 68)
    print("  DONE -- Phase 6 MCDM Ranking")
    print(f"  Saved: {OUT_TOPK}")
    print(f"  Saved: {OUT_FULL}")
    print(f"  Saved: {OUT_MC}")

    # Check if same PCM wins all clusters
    ...
    if (~disagree).any():
        print(f"\n  [NOTE] Borda/Copeland disagree on #1 for cluster(s) "
              f"{disagree[~disagree].index.tolist()} -- report both.")

    print("=" * 68)
    print("\nNext: python 09_recommendation_cards.py")
