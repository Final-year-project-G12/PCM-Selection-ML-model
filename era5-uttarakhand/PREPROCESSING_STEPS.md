...
Runs directly on `climate_uttarakhand_points.csv` (02's output). Nothing here
...
| Plot | What it checks |
|---|---|
| A. Point map | Are the population points actually covering Uttarakhand, sized/colored by population as intended |
...
| E. Seasonal boxplots | Sanity check against known Uttarakhand climatology (hot foothill/Terai summer Apr–Jun, southwest monsoon Jun–Sep, cold high-altitude winter Dec–Feb) |
...
**Outputs:** `uttarakhand_cleaned_physical.csv` (feeds 04b), `uttarakhand_cleaned_scaled.csv` + `scalers.pkl` (for later ML/DRL use), `qc_report.txt`, correlation/VIF/Yeo-Johnson CSVs, two diagnostic PNGs.

---

## `04b_climate_signature.py` — Phase 3, Climate Signature Construction

Reads the **physical-units** file only, plus `02b`'s
`tier2_signature_uttarakhand.csv`. Collapses each point's entire 10-year,
...
Runs now, on Uttarakhand's own signature matrix alone — doesn't wait on
any other state. Fits a **Gaussian Mixture Model** (soft/probabilistic
clustering — not K-Means; the plan doc explicitly rejects K-Means here
because climate is a continuous gradient, not hard-boundaried), selects K
by BIC with a stated realistic silhouette acceptance band of 0.15-0.40 (a
very high silhouette would mean the signature collapsed to 1-2 dominant
variables, not that regimes are unusually crisp — say so if yours comes
out high, given only 45 points). Outputs both hard cluster labels and full
soft membership probabilities per point, plus population-weighted cluster
profiles.

`05_cluster_regions.py` (the original multi-state script) still exists,
already state-parameterised, for later — if you add Rajasthan/Assam/
Tamil Nadu's Phase 3 output, that's what you switch to. Nothing downstream
(Phase 5/6) needs to change if/when you do, since the output shapes match.
...
```
02_combine_uttarakhand.py      -> climate_uttarakhand_points.csv       (Phase 1, done)
...
04_preprocess_uttarakhand.py    -> Phase 2                              (this doc)
04b_climate_signature.py        -> Phase 3                              (this doc)
05_cluster_uttarakhand.py       -> Phase 4, single-state                (this doc — runs now)
05_cluster_regions.py           -> Phase 4, multi-state                 (waits on other states)
```
