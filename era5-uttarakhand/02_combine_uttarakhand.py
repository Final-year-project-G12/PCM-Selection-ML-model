"""
COMBINE — UTTARAKHAND POPULATION POINTS  (ERA5 + NASA POWER cross-check)
=============================================================================
Reads  :
...
Writes :
  data/processed/climate_uttarakhand_points.csv
...
This replaces the old named-location and opt-in full-grid combine logic
entirely — nothing else in the repo reads the old output, so there's no
need to keep both schemes running side by side.
...
HOW TO RUN:
  python 02_combine_uttarakhand.py

Safe to re-run — overwrites climate_uttarakhand_points.csv with fresh data.
"""
...
def load_all_files():
    ...
    for year in YEARS:
        ...
        accum_ds[year]   = {}
        for month in MONTHS:
            fi = os.path.join(INPUT_DIR, f"era5_UK_points_{year}_{month}_instant.nc")
            fa = os.path.join(INPUT_DIR, f"era5_UK_points_{year}_{month}_accum.nc")
...
if __name__ == "__main__":
    ...
    print("=" * 68)
    print("  ERA5 + NASA POWER COMBINE — UTTARAKHAND POPULATION POINTS")
    ...
    instant_count = sum(
        ...
        if os.path.exists(os.path.join(INPUT_DIR, f"era5_UK_points_{y}_{m}_instant.nc")))
    accum_count = sum(
        1 for y in YEARS for m in MONTHS
        if os.path.exists(os.path.join(INPUT_DIR, f"era5_UK_points_{y}_{m}_accum.nc")))
    expected = len(YEARS) * len(MONTHS)
    power_count = sum(
    ...
    if instant_count == 0 and accum_count == 0:
        print("\n  ⚠️  No ERA5 files found — run 01_download_era5_uttarakhand.py first.")
    if power_count == 0:
    ...
    print(f"  Output           : {COMBINED_POINTS_FILE}")
    print("=" * 68)
    print("\nNext: load climate_uttarakhand_points.csv for training / cross-checking.")
