...
==========================
POST-PREPROCESSING QA — run this AFTER 04_preprocess_uttarakhand.py, on its
output (uttarakhand_cleaned_physical.csv). 03_plots_raw.py checked the RAW
...
print("=" * 68)
print("  Post-Preprocessing QA Plots — Uttarakhand")
print(f"  Input  : {PHYSICAL_FILE}")
print(f"  Output : {POST_PLOT_DIR}/")
print("=" * 68)

if not PHYSICAL_FILE.exists():
    raise FileNotFoundError(f"{PHYSICAL_FILE} not found — run 04_preprocess_uttarakhand.py first.")
...
if QC_REPORT.exists():
    ...
    print(f"  [SKIP] {QC_REPORT} not found — rerun 04_preprocess_uttarakhand.py to regenerate it.")
...
print("  go back and check 04_preprocess_uttarakhand.py's qc_report.txt")
