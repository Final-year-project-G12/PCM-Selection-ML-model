...
===========================
ERA5 Uttarakhand — Interactive Explorer (Streamlit)

Adapted from the original state-wide hourly/city explorer used by the very
first (pre-points) version of this pipeline to
...
View 1 — Raw Time Series
  ...
  bounds from 04_preprocess_uttarakhand.py) marked in orange. Range slider.
...
DATA REQUIRED
-------------
  data/processed/climate_uttarakhand_points.csv          (02's output — raw)
  data/preprocessed/uttarakhand_cleaned_physical.csv      (04's output — processed)
...
# ═══════════════════════════════════════════════════════════
st.set_page_config(
    page_title="ERA5 Uttarakhand — Interactive Explorer",
    page_icon="🌞",
    layout="wide",
...
PROCESSED_FILE = PREPROCESSED_DIR / "uttarakhand_cleaned_physical.csv"
OUT_DIR = PREPROCESSED_DIR.parent / "plots" / "interactive_explorer"
os.makedirs(OUT_DIR, exist_ok=True)

if not os.path.exists(RAW_FILE):
    st.error(f"**File not found:** `{RAW_FILE}`\n\nRun `02_combine_uttarakhand.py` first.")
    st.stop()
if not os.path.exists(PROCESSED_FILE):
    st.warning(
        f"**Processed file not found:** `{PROCESSED_FILE}`\n\n"
        "Run `04_preprocess_uttarakhand.py` first. Raw-only views below still work."
    )

EVENT_ORDER = ["sunrise", "noon", "sunset"]
EVENT_COLORS = {"sunrise": "#f9c74f", "noon": "#4cc9f0", "sunset": "#f3722c"}

# Physical bounds — same dict as 04_preprocess_uttarakhand.py, used only to
...
# ═══════════════════════════════════════════════════════════
st.sidebar.markdown(
    "<h2 style='color:#4cc9f0;margin-bottom:4px'>🌞 ERA5 Uttarakhand</h2>"
...
# ═══════════════════════════════════════════════════════════
    ...
    if df_proc is None:
        st.error("Processed file not loaded — run `04_preprocess_uttarakhand.py` first.")
    else:
