...
climate_signature_uttarakhand.csv (assuming roughly-normal day-to-day
...
OUTPUT : data/processed/clustering/cluster_profiles_uttarakhand.csv
...
# clear-sky insolation can plausibly deliver up to ~70C (roughly
# consistent with Al-Mamun2023's cited FPC 25-100C operating band); this
# is a generic collector-physics ceiling, not a Uttarakhand-specific
# number — if anything, this state's higher-altitude clusters see LESS
# reliable clear-sky access than the plains (more cloud/fog persistence,
# not less), which is exactly what kt_mean/kt_std already capture per
# cluster below. On a genuinely poor day, delivery drops toward something
# closer to the absolute floor this project already uses (42C).
...
def main():
    ...
    if not PROFILE_FILE.exists():
        print(f"\n  ERROR: {PROFILE_FILE} not found — run 05_cluster_uttarakhand.py first.")
        return

    profiles = pd.read_csv(PROFILE_FILE)
    if "kt_mean" not in profiles.columns or "kt_std" not in profiles.columns:
        print("\n  ERROR: cluster_profiles_uttarakhand.csv is missing kt_mean/kt_std — "
    ...
    print("column from cluster_profiles_uttarakhand.csv or re-run 05 to regenerate it.)")
