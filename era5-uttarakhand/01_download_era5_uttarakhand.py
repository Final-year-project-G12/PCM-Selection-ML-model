"""
ERA5 DOWNLOAD — UTTARAKHAND POPULATION-WEIGHTED POINTS  (sun-event-aligned, 10-year history)
=============================================================================
...
Uttarakhand, not the full state — at UTC hours computed from suntimes.csv
...
Cross-midnight UTC dates are real here, not hypothetical: Uttarakhand's
...
HOW TO RUN:
  ...
  3. python 01_download_era5_uttarakhand.py
...
# DHI is computed in 02_combine_uttarakhand.py as: DHI = GHI - DNI × cos(SZA)
# ═══════════════════════════════════════════════════════════
...
UK_BBOX, N_POINTS = load_points_bbox()
...
def main():
    print("\n" + "═" * 68)
    print("  ERA5 Uttarakhand — Population Points Download  (sun-event hours, 10 years)")
    print(f"  Started : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Points  : {N_POINTS}  (from {POPULATION_GRID_FILE.name})")
    print(f"  BBox    : N={UK_BBOX[0]:.2f} W={UK_BBOX[1]:.2f} "
          f"S={UK_BBOX[2]:.2f} E={UK_BBOX[3]:.2f}")
    ...
    print("\nNext step: run  01b_download_nasapower.py  (if not already done), "
          "then  00_unzip_accum.py,  then  02_combine_uttarakhand.py")
