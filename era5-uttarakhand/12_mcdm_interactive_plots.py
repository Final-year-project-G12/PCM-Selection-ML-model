...
def main():
    ...
    if hasattr(px, "scatter_map"):
        # Plotly >= 6.0 removed scatter_mapbox in favor of scatter_map
        # (MapLibre-based, no Mapbox account/token needed).
        plot = px.scatter_map(
            map_points,
            lat="lat",
            lon="lon",
            ...
            title="Uttarakhand MCDM PCM Recommendations by Climate Cluster",
            color_discrete_sequence=palette,
        )
        plot.update_layout(map_style="open-street-map", margin={"l": 0, "r": 0, "t": 45, "b": 0})
    else:
