...
You are working on Uttarakhand only right now. That cross-state comparison
isn't required for Objective 1 to stand on its own: the objective is
"cluster meteorological data and identify Top-2/Top-3 PCM candidates per
climatic regime" — nothing in the objective statement requires those
regimes to span multiple states. Clustering Uttarakhand's population points
into their own regimes still gives you exactly what you need: different
PCM recommendations for different LOCATIONS within Uttarakhand (e.g. the
high-altitude Himalayan belt around Chamoli/Pithoragarh vs. the Doon
Valley around Dehradun vs. the Terai plains around Udham Singh Nagar/
Haridwar are very plausibly different regimes — elevation alone spans
roughly 200-2000m of populated terrain here).
...
the boundary between "high-hill" and "valley/plains" Uttarakhand is not a
hard line, and a point near that boundary genuinely has partial membership
in both. Soft membership probabilities are kept and are what Phase 5/6
should read for boundary points.
...
OUTPUT : data/processed/clustering/
           bic_selection_uttarakhand.csv
           cluster_assignments_uttarakhand.csv   <- soft membership probabilities
           cluster_profiles_uttarakhand.csv      <- population-weighted profile
...
HOW TO RUN:
  python 05_cluster_uttarakhand.py
"""
...
def main():
    print("=" * 68)
    print("  Phase 4 — Climate Regime Clustering (Gaussian Mixture) — Uttarakhand only")
    print("=" * 68)
    ...
    k_path = OUT_DIR / "bic_selection_uttarakhand.csv"
    ...
    km_table.to_csv(OUT_DIR / "kmeans_comparison_uttarakhand.csv", index=False)
    ...
    assign_path = OUT_DIR / "cluster_assignments_uttarakhand.csv"
    ...
    profile_path = OUT_DIR / "cluster_profiles_uttarakhand.csv"
    ...
    ax.set_title(f"Uttarakhand Climate Regimes — GMM, K={k_final_safe}")
    ax.set_xlabel("Longitude"); ax.set_ylabel("Latitude")
    ax.set_aspect("equal")
    plt.tight_layout()
    plt.savefig(OUT_DIR / "cluster_map_uttarakhand.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved: {OUT_DIR / 'cluster_map_uttarakhand.png'}")
    ...
    print("\nNext (Phase 5): build the PCM property database (40+ candidates, 42-70C "
          ...
          "from cluster_profiles_uttarakhand.csv, then rank with TOPSIS + GRA "
