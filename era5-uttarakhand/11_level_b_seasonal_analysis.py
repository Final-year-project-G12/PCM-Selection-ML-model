...
Level A (05_cluster_uttarakhand.py) clusters points across all 10 years
...
DIFFERENT PCM in July than in March?" — Uttarakhand spans roughly
200-2000m of populated terrain (Terai plains around Udham Singh Nagar/
Haridwar up to the Himalayan belt around Chamoli/Pithoragarh), so the
same cluster can see a much colder, fog/western-disturbance-affected
winter and a monsoon (Jun-Sep) with very different solar reliability —
exactly the situation where a seasonal PCM flip is plausible.

The plan calls this "nearly free" because season/season_code are already
columns in your cleaned data from 04_preprocess_uttarakhand.py. This script
...
INPUT  : data/preprocessed/uttarakhand_cleaned_physical.csv   (04's output;
         ...
         data/processed/pcm/pcm_database_uttarakhand.csv
...
def main():
    print("=" * 68)
    print("  Phase 4, Level B — Seasonal PCM Sensitivity — Uttarakhand")
    print("=" * 68)
    ...
    all_rows = []
    md_lines = ["# Level B — Seasonal PCM Sensitivity (Uttarakhand)\n"]
    ...
    if len(result_df):
        ...
        if n_flips > 0:
            ...
            print("  No flips detected — also a valid finding: it means the corrected "
                  ...
                  "is robust to Uttarakhand's seasonal swings, which is itself worth "
