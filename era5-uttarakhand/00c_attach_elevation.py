"""
ELEVATION ATTACHMENT — UTTARAKHAND POPULATION POINTS
=============================================================================
Population points carry no elevation field, so 02_combine_uttarakhand.py
...
That is exactly the case here, more so than any other state in this
project. Uttarakhand's populated terrain runs from the Terai plains
(~200-300 m around Haridwar / Udham Singh Nagar) through the Doon Valley
up into the high Himalayan belt (Chamoli / Pithoragarh / Uttarkashi,
several thousand metres, with peaks past 7,000 m), so one flat default is
wrong by kilometres for a large fraction of the sampling points — a
required repair for this state, not the optional polish it was for
comparatively flat Rajasthan (200-500 m).
...
envelope 01_download_era5_uttarakhand.py uses — and attaches a per-point
...
Output:
  data/raw/era5/invariant/era5_UK_geopotential.nc   (raw cache)
...
with sharp terrain — Uttarakhand's 200m-7000m+ range is the textbook case —
a single cell value smooths out real local relief, and a deep valley town
can sit far below its cell mean. This is an accepted, documented
limitation, not something this script tries to fix further — but a
cell-mean elevation is still far closer to the truth than one flat number
for the whole state.
"""
...
# 01_download_era5_uttarakhand.py / 02_combine_uttarakhand.py use.
...
BBOX_PAD_DEG = 0.5   # same padding 01_download_era5_uttarakhand.py uses
...
# ═══════════════════════════════════════════════════════════
# BBOX  (same logic as 01_download_era5_uttarakhand.py's load_points_bbox)
# ═══════════════════════════════════════════════════════════
...
def open_nc(fpath):
    """Same multi-engine fallback as 02_combine_uttarakhand.py's open_nc."""
