...
climate_uttarakhand_points.csv — this script never writes back to it.

INPUT  : data/processed/climate_uttarakhand_points.csv  (02_combine output)
OUTPUTS:
  data/processed/era5_power_agreement_uttarakhand.csv
  outputs/qc_era5_power_scatter_uttarakhand.html
  outputs/bias_decision_uttarakhand.txt
...
def build_ghi_scatter(df, pairs):
    ...
    fig.update_layout(title="ERA5 vs NASA POWER GHI by season — Uttarakhand", height=850, width=950)
    out = OUTPUTS_DIR / "qc_era5_power_scatter_uttarakhand.html"
...
def write_decision_txt(branch, info, qmap_results, diagnostics):
    ...
    out = OUTPUTS_DIR / "bias_decision_uttarakhand.txt"
...
def main():
    print("=" * 68)
    print("  CROSS-SOURCE AGREEMENT ANALYSIS — Uttarakhand (ERA5 vs NASA POWER)")
    if not COMBINED_POINTS_FILE.exists():
        raise SystemExit(f"ERROR: {COMBINED_POINTS_FILE} not found — run 02_combine_uttarakhand.py first.")
    df, pairs = load_combined()
    agreement_df = build_agreement_table(df, pairs)
    agreement_path = PROCESSED_DIR / "era5_power_agreement_uttarakhand.csv"
