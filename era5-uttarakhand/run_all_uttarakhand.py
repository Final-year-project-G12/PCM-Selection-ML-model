...
=============================================================================
Runs every stage of the Uttarakhand (population-weighted points) pipeline,
...
group below):
  ...
  — the multi-state (Tamil Nadu + Rajasthan + Assam + Uttarakhand
  combined) clustering design, kept on standby for when a second state's
  Phase 3 output exists alongside this one. Uttarakhand-only clustering
  uses 05_cluster_uttarakhand.py instead (see CORE_SCRIPTS below); this
  file is intentionally not part of the single-state run.
...
KNOWN GAP — read before running the CORE chain unattended:
  ...
  outside era5-uttarakhand-pipeline/ and uttarakhand_pipeline/). As of writing,
...
07b which is marked optional below — see that entry):
  1.  02_combine_uttarakhand.py       — Phase 1 final step: merges ERA5 +
                                       NASA POWER at sun-events ->
                                       climate_uttarakhand_points.csv, the
  ...
  3.  04_preprocess_uttarakhand.py    — Phase 2: 13-step QC pipeline ->
                                       uttarakhand_cleaned_physical.csv,
  ...
  4.  04b_climate_signature.py      — Phase 3: collapses each point's
                                       ...
                                       climate_signature_uttarakhand.csv,
                                       what 05 clusters
  5.  05_cluster_uttarakhand.py       — Phase 4: GMM clustering ->
                                       cluster_profiles_uttarakhand.csv
...
chain; each one's failure is logged and does NOT stop the run):
  ...
  05d_plots_comprehensive.py, 12_mcdm_interactive_plots.py

HOW TO RUN:
  python run_all_uttarakhand.py                 # core pipeline only (default)
  python run_all_uttarakhand.py --with-optional  # core + diagnostic/plot scripts
  python run_all_uttarakhand.py --dry-run        # print the order, run nothing
  python run_all_uttarakhand.py --include-setup  # ALSO run the raw-data
                                                # acquisition scripts FIRST —
                                                # read the warning above and
                                                # in --include-setup's own
                                                # section below before using
  python run_all_uttarakhand.py --from 05_cluster_uttarakhand.py
...
OPTIONAL_SCRIPTS = [
    ...
    "12_mcdm_interactive_plots.py",  # presentation maps; run after 08_mcdm_ranking.py
]
...
def main():
    parser = argparse.ArgumentParser(description="Run the Uttarakhand points pipeline end-to-end, in order.")
    ...
    parser.add_argument("--from", dest="from_script", default=None,
                         help="Resume the CORE chain starting at this script name "
                              "(e.g. 05_cluster_uttarakhand.py), skipping everything before it. "
    ...
    print("=" * 68)
    print("  UTTARAKHAND POINTS PIPELINE — RUN ORDER")
    print("=" * 68)
    if setup:
