"""
POPULATION-WEIGHTED GRID POINTS — UTTARAKHAND
=============================================================================
...
def load_uttarakhand_boundary():
    ...
    uk = gdf[gdf["NAME_1"] == "Uttarakhand"]
    if uk.empty:
        raise RuntimeError(
            "Uttarakhand not found in GADM NAME_1 column — "
            f"available values: {sorted(gdf['NAME_1'].unique())}")
    return uk.geometry.iloc[0]
...
def aggregate_population(raster_path, boundary_geom):
    ...
    (row x col) index grids — at WorldPop's 100m resolution, Uttarakhand is
...
def main():
    print("=" * 68)
    print("  Population-Weighted Grid Points — Uttarakhand")
    print("=" * 68)

    print("\n[1/4] Uttarakhand boundary (GADM level-1) ...")
    boundary = load_uttarakhand_boundary()
    ...
    print(f"  {len(grid_df)} populated cells inside Uttarakhand boundary")
    ...
    selected["point_id"] = [f"UKP_{i+1:04d}" for i in range(len(selected))]
