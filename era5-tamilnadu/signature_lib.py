...
def compute_hsi_sunrise(t_sunrise_mean, rh_sunrise_mean):
    """Thom's (1959) Discomfort Index (HSI) evaluated at the coldest/
    condensation-critical instant of the day (sunrise), as a function of
    T_sunrise_mean and RH_sunrise_mean directly. This is literally Thom's
    Discomfort Index / Temperature-Humidity Index (Thom, E.C., "The
    Discomfort Index", Weatherwise 12(2), 1959) — an established index, not
    a bespoke "humidity stress index"; cite it as Thom's Discomfort Index in
    any write-up. The `HSI_sunrise` column name is kept for continuity.
        HSI = T - 0.55*(1 - RH/100)*(T - 14.5)      [T in C, RH in %]
