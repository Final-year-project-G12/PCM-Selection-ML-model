"""
07_plots_comprehensive.py
============================
ERA5 Tamil Nadu — Comprehensive Visualization Batch
(the point/event-schema equivalent of your old 05_plot_tamilnadu.ipynb)

Delivered as a plain script rather than a notebook on purpose — it runs
identically via `python 07_plots_comprehensive.py` or cell-by-cell if you
...
----------
  ...
  python 07_plots_comprehensive.py
...
USE_PROCESSED = True   # False -> plots raw climate_tamilnadu_points.csv instead
...
DATA_FILE = (PREPROCESSED_DIR / "tamilnadu_cleaned_physical.csv") if USE_PROCESSED else COMBINED_POINTS_FILE
...
print("=" * 68)
print("  ERA5 Tamil Nadu — Comprehensive Visualization Batch")
...
ax.plot(state_daily["date"], state_daily["era5_GHI"].rolling(7, min_periods=1).mean(),
        color="#f9c74f", linewidth=2.6, label="Tamil Nadu mean (7-day rolling)")
