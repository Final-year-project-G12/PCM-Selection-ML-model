"""
Tamil Nadu PCM Pipeline - Objective 1 Plot Generator
Generates all 13 required plots (static PNG + interactive Plotly/Folium)
Output: data/plots/tamilnadu_objective1/
"""
...
RAW_CSV      = os.path.join(BASE,"data","processed","climate_tamilnadu_points.csv")
PHYSICAL_CSV = os.path.join(BASE,"data","preprocessed","tamilnadu_cleaned_physical.csv")
CLUSTERS     = os.path.join(BASE,"data","processed","clustering","cluster_assignments_tamilnadu.csv")
...
FULL_SCORES  = os.path.join(BASE,"data","processed","pcm","mcdm_full_rankings.csv")
# BUG FIX (2026-09-16): was "mcdm_full_scores_by_cluster.csv", the OLD
# 5-criteria script's output filename. The current unified 8-criteria
# 08_mcdm_ranking.py writes "mcdm_full_rankings.csv" instead -- the old file
# was still present on disk (stale, k=5/Tm_target=57C data) so this loaded
# silently without error, just wrong/stale data, while every other plot in
# this script used fresh data. p07()'s ensure_ranks() call recomputes the
# lowercase topsis_rank/gra_rank/promethee_rank/vikor_rank columns from
# topsis_score/gra_grade/promethee_flow/vikor_Q when they're missing (the
# new file uses TOPSIS_rank/PROMETHEE_II_rank/... instead), so no further
# column-name changes were needed once this path pointed at the right file.
...
def p01():
    ...
    plt.suptitle("Tamil Nadu - Raw vs Preprocessed Solar Radiation (GHI)",fontsize=13)
...
def p02():
    ...
    ax.set(title="Tamil Nadu - Climate Regime Map\n(GMM clusters per grid point)",xlabel="Longitude E",ylabel="Latitude N",xlim=[76.5,80.5],ylim=[8,14])
    ...
    fig_px=px.scatter(df,x="lon",y="lat",color=df["cluster_id"].astype(str),
                      ...
                      title="Tamil Nadu - Climate Regime Map",labels={"color":"Cluster"},template="plotly_white",
...
def p04():
    ...
    plt.suptitle("Tamil Nadu - PCM Feasibility Filter: Candidates Highlighted",fontsize=13,fontweight="bold"); plt.tight_layout(); sfig("04_feasible_candidates_highlighted.png")
...
def p05():
    ...
    fig_px=px.bar(cnt,x=cnt["cluster_id"].astype(str),y="n",text="n",color=cnt["cluster_id"].astype(str),
                  title="Feasible PCM Candidates per Climate Regime (Tamil Nadu)",template="plotly_white",
                  color_discrete_sequence=px.colors.qualitative.Set1)
...
def p06():
    ...
    plt.suptitle("Tamil Nadu - PCM Feasibility (Scatter and Count)",fontsize=13,fontweight="bold"); plt.tight_layout(); sfig("06_pcm_feasibility_scatter_and_survivors.png")
    ...
    ax2.set(title="PCM Feasibility Scatter - Tamil Nadu",xlabel="Melting Temp (C)",ylabel="Latent Heat (kJ/kg)"); ax2.legend(); ax2.grid(alpha=0.25)
    ...
    ax3.set(title="Survivors per Cluster - Tamil Nadu",xlabel="Cluster ID",ylabel="Feasible PCM count"); ax3.grid(alpha=0.3,axis="y")
...
def p07():
    ...
    # Legend placed OUTSIDE the axes (bbox_to_anchor beyond x=1): a
    # BUG FIX — loc="upper right" alone draws the legend box INSIDE the
    # plot area, and because ranks converge toward 1 (best) at the
    # right-hand end of the x-axis (PROMETHEE/VIKOR/CONSENSUS), that box
    # sat directly on top of the densest cluster of crossing/converging
    # lines, visually bleaching their connecting segments under its
    # translucent white background (looked like floating, disconnected
    # points even though every line was fully continuous in the data).
    ax_s.legend(fontsize=6,ncol=2,loc="upper left",bbox_to_anchor=(1.01,1.0),borderaxespad=0)
    ax_s.grid(alpha=0.25); fig_s.tight_layout(); sfig("07_bump_chart_ranks.png")
    ...
    for cid in cid_list:
        ...
        # same legend-occlusion bug fix as the combined chart above: legend
        # drawn outside the axes instead of on top of the converging lines
        ax_sc.legend(fontsize=8,loc="upper left",bbox_to_anchor=(1.01,1.0),borderaxespad=0)
...
def p08():
    ...
    plt.suptitle("Tamil Nadu - Rank Correlation Between MCDM Methods",fontsize=13,fontweight="bold"); plt.tight_layout(); sfig("08_method_rank_correlation_heatmap.png")
    spd=pd.DataFrame(sp,index=labs,columns=labs)
    fig_px=px.imshow(spd,text_auto=".2f",zmin=-1,zmax=1,color_continuous_scale="RdYlGn",
                     title="Tamil Nadu - Spearman rho Between MCDM Ranking Methods",template="plotly_white")
...
def p09():
    ...
    ax.set_title("Tamil Nadu - Monte Carlo Top-3 Inclusion Probability\n(5000 draws per cluster)",fontsize=13,fontweight="bold")
    ...
    fig_px=px.bar(df,x=vals,y="name",color=df["cluster_id"].astype(str),orientation="h",
                  title="Monte Carlo Top-3 Inclusion Probability (Tamil Nadu)",template="plotly_white",
...
def p10():
    ...
    plt.suptitle("Tamil Nadu - Rank Reversal Frequency Across MCDM Methods",fontsize=13,fontweight="bold"); plt.tight_layout(); sfig("10_rank_reversal_violin_bar.png")
    if not ld.empty:
        fig_px=px.violin(ld,x="Method",y="Rank",color="Cluster",box=True,points="all",
                         title="Rank Reversal - Violin Plot (Tamil Nadu)",template="plotly_white",
                         color_discrete_sequence=px.colors.qualitative.Set1)
...
def p12():
    ...
    plt.suptitle("Tamil Nadu - Representative Day-Night Tank Temperature and Melt-Fraction",fontsize=13,fontweight="bold")
    ...
    fig_int.update_layout(height=400*nc,title="Tamil Nadu - Tank Temperature and Melt Fraction",template="plotly_white")
...
def p13():
    ...
    plt.suptitle("Tamil Nadu - Recommended PCM per Climate Cluster\n(Consensus MCDM Ranking, Top-3)",fontsize=13,fontweight="bold")
    ...
    fig_px=px.bar(top,x=top["cluster_id"].astype(str),y="latent_heat_kJ_kg",color="name",barmode="group",hover_data=props,
                  title="Tamil Nadu - Recommended PCM by Climate Cluster (Top-3, Consensus Rank)",template="plotly_white",
