"""
Comparison Plots - Tamil Nadu PCM Pipeline
==========================================
...
SIG_CSV = os.path.join(BASE,"data","processed","signatures","climate_signature_tamilnadu.csv")
...
CPROF   = os.path.join(BASE,"data","processed","clustering","cluster_profiles_tamilnadu.csv")
...
if sig is not None and clu is not None:
    ...
    if ghi_col:
        ...
        ax.set(xlabel="Cluster",ylabel=f"{ghi_col} (mean +/- std)",title="Comparison 1: Mean GHI by Climate Regime (Tamil Nadu)")
...
if sig is not None and clu is not None:
    ...
    if t_col and feas is not None and "Tm_target_C" in feas.columns:
        ...
        ax.set(xlabel=f"Cluster Mean Temperature ({t_col}) (C)",ylabel="PCM Target Melting Point (C)",title="Comparison 2: Cluster Temperature vs PCM Tm Target (Tamil Nadu)")
...
if topk is not None:
    ...
    if len(methods)>=2:
        ...
        plt.suptitle("Comparison 3: MCDM Methods Side-by-Side (Tamil Nadu)",fontsize=13,fontweight="bold"); plt.tight_layout(); sfig("03_comparison_mcdm_methods.png")
...
if mc is not None and topk is not None and "top3_inclusion_probability" in mc.columns:
    ...
    ax.set(xlabel="Consensus Rank",ylabel="Top-3 Inclusion Probability (%)",title="Comparison 4: Monte Carlo Stability vs MCDM Consensus Rank (Tamil Nadu)")
    ...
    ax.set(xlabel="Consensus Rank",ylabel="Top-3 Prob (%)",title="Comparison 4: Monte Carlo Stability vs Consensus Rank (Tamil Nadu)")
    ax.legend(fontsize=9); ax.grid(alpha=0.25); sfig("04_comparison_mc_vs_rank.png")
...
if feas is not None and "latent_heat_kJ_kg" in feas.columns:
    ...
    ax.set(xlabel="Latent Heat (kJ/kg)",ylabel="Density",title="Comparison 5: Latent Heat Distribution - All vs Feasible Survivors (Tamil Nadu)")
...
if phys is not None and topk is not None and "hours_target_met_per_year" in phys.columns:
    ...
    if "consensus_rank" in mg6.columns and not mg6.empty:
        ...
        plt.suptitle("Comparison 6: Physics Validation vs MCDM Ranking (Tamil Nadu)",fontsize=13,fontweight="bold"); plt.tight_layout(); sfig("06_comparison_physics_vs_rank.png")
...
if topk is not None and "consensus_rank" in topk.columns:
    ...
    if len(props)>=2:
        ...
        plt.suptitle("Comparison 7: Top PCM Properties per Cluster (Tamil Nadu) - Rank #1",fontsize=13,fontweight="bold"); plt.tight_layout(); sfig("07_comparison_cross_cluster_top_pcm.png")
    ...
    if len(score_cols)>=2:
        ...
        if not rdf.empty:
            ...
            ax.set(xlabel=f"Weight on {c1.replace('_score','').replace('_grade','').upper()} (remainder on {c2.replace('_score','').replace('_grade','').upper()})",
                   ylabel="Combined Rank",title="Comparison 8: Rank Sensitivity to Weight Perturbation (Tamil Nadu)")
