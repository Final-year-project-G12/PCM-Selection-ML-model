...
=============================================================================
PHASE 4 — LEVEL B: TEMPORAL (SEASONAL) REGIME-SHIFT CLUSTERING, TAMIL NADU
(Objective1_PCM_Climate_Framework_Plan_v3, §7.2)

PORTED FROM Rajasthan's 05a_level_b_regime_shift_rajasthan.py (2026-09-08),
which in turn was extracted from that state's Level-A script. Tamil Nadu had
no Level-B regime-shift clustering at all before this; adding it here gives
both states the same structure and so the two analyses that were
...
INPUTS:
  data/processed/climate_tamilnadu_points.csv  (per-point-per-season Tier 1
...
OUTPUTS:
  data/processed/clustering/bic_selection_tamilnadu_levelB.csv
  data/processed/clustering/cluster_assignments_tamilnadu_levelB.csv
  data/processed/clustering/level_b_feature_importance_tamilnadu.csv
  data/processed/clustering/level_b_season_tautology_tamilnadu.csv
  data/processed/clustering/level_b_season_contingency_tamilnadu.csv
  outputs/qc_level_b_regime_shift_sankey_tamilnadu.html   (NEW 2026-09-08 —
...
HOW TO RUN:
  python 05a_level_b_regime_shift_tamilnadu.py
"""
