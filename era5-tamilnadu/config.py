"""
Shared paths and CDS API settings for the ERA5 Tamil Nadu pipeline
(population-weighted points, sun-event-aligned, 10-year history).

Mirrors the structure of the Rajasthan pipeline (era5-rajasthan/config.py)
exactly, so both regions produce directly comparable outputs for the later
...
# Old uniform full-state-grid pipeline (kept only so 00_unzip_accum.py can
# still find/fix any leftover files from that earlier run — not used by
# the new points pipeline below).
RAW_GRID_DIR = RAW_ERA5_DIR / "grid"
DOWNLOAD_STATUS_FILE = RAW_ERA5_DIR / "download_status.csv"

# Population-weighted points pipeline (the one actually used from here on).
...
COMBINED_POINTS_FILE = PROCESSED_DIR / "climate_tamilnadu_points.csv"
...
def load_cds_credentials():
    """Read url and key from .cdsapirc, or from CDSAPI_URL/CDSAPI_KEY env vars."""
    import os
    ...
    if not CDSAPI_RC.is_file():
        raise FileNotFoundError(
            ...
            "Add a .cdsapirc file in this folder, or set CDSAPI_URL and CDSAPI_KEY."
        )
    ...
    if not text:
        raise ValueError(
            ...
            "or set CDSAPI_URL and CDSAPI_KEY."
        )
