...
Run order:
```
00c_attach_elevation.py         (Phase 1 — added 2026-09-16, run before 02_combine)
02_combine_tamilnadu.py         (Phase 1 — now uses real per-point elevation)
...
05_cluster_tamilnadu.py         (Phase 4 — clustering, TN only, k auto-selected)
```
...
- **Tm_target and L_required** — `Tm_target = T_delivery + delta_T_approach`
  ...
  constant across all points by design. **`T_delivery` corrected 2026-09-13
  from 50°C to 60°C** (the night-discharge formula's "300L/7h" capability
  figure, Avargani et al. 2021, is only validated AT 60±2°C delivery — using
  it at 50°C was citing a number outside what that source actually tested),
  so `Tm_target = 60 + 7 = **67°C**`, not 57°C as earlier drafts of this
  doc said. See `CHANGELOG.md`'s 2026-09-16 entry — this pipeline copy was
  resolving to a stale, uncorrected `pcm_shared_config.py` until then.
...
**elevation note (resolved 2026-09-16):** `00c_attach_elevation.py` now
attaches real per-point elevation (ERA5 time-invariant geopotential,
-0.0m–1,283.4m across the 133 points, mean 282.4m) before `02_combine`
runs, replacing the flat 150m proxy this note used to describe. The ERA5
~28km grid still smooths the true Nilgiris peak (~2,600m) to a cell-mean
value — an accepted, documented limitation, not something further
corrected.
...
soft membership probabilities, `cluster_profiles_tamilnadu.csv` population-
weighted per regime), so Phases 5–8 need no change.
