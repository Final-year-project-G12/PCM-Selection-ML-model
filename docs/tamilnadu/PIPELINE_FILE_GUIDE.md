...
### `00c_attach_elevation.py` (added 2026-09-16)
Downloads ERA5's time-invariant surface geopotential field (one CDS
request — geopotential doesn't vary by date, unlike the weather fields
`01` downloads) and attaches real per-point `elevation_m` to
`population_grid_points.csv`, replacing the flat 150m default
`02_combine_tamilnadu.py` used before this script existed. **Output:**
`elevation_m` column added in place (-0.0m–1,283.4m across the 133 points,
mean 282.4m — the ERA5 ~28km grid smooths the true Nilgiris peak, an
accepted limitation, not a bug). Run before `02_combine_tamilnadu.py`;
safe to re-run (skips both the download and the attach step if already done).
...
- **`docs/era5_tamilnadu/00_MASTER_OVERVIEW.md`** — current status table +
  what's left, updated each round (this is where `NEXT_STEPS.md` used to
  live before it was a completed, fully-superseded sprint plan and was
  deleted 2026-09-16 — see `CLEANUP_CANDIDATES.md`).
