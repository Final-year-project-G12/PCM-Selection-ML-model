...
| Humidity-stress index (HSI) | `04b_climate_signature.py` | Thom (1959), *Weatherwise* 12(2) — Discomfort/THI formulation | Weak — the coded formula (`RH_mean × mean(fraction of events with (Ta−Td)<3K)`, a dew-point-proximity index) does not match a Thom THI formulation, and HSI is **not** the value actually used by the corrosion-veto check in `07_feasibility_filter_final.py` (which uses `RH_mean_mean` instead — see `08_PHASE_6_AUDIT.md`). Not currently load-bearing for any screening decision. |
