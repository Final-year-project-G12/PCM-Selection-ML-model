...
| 4 — Regime Clustering | N1 | GMM-discovered regimes (k=3, statistically selected, not hand-picked); external validation specified but not yet wired in — **N1's "discovered, not hand-picked" claim is currently supported only by internal statistical measures, not yet by an external classification cross-check** |
...
| 7 — Physics Validation (not yet implemented) | N5 | Would independently validate the MCDM ranking against simulated solar fraction — until built, N5 is a stated intent, not yet a delivered result |
| 8 — Recommendation Cards (not yet implemented) | (packaging) | Aggregates N1–N5's evidence into the final deliverable format |
