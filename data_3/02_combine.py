...
def compute_sunrise_sunset(df, lat, lon, altitude_m):
    ...
    location = pvlib.location.Location(latitude=lat, longitude=lon,
                                       altitude=altitude_m, tz="Asia/Kolkata")
    dates = pd.DatetimeIndex(df["timestamp"].dt.normalize().unique()).tz_localize("UTC").tz_convert("Asia/Kolkata")
...
def unzip_if_needed(filepath):
    with open(filepath, 'rb') as f:
        if f.read(4)[:2] != b'PK': return [filepath]
    ...
    with zipfile.ZipFile(zip_path, 'r') as z:
        nc_inside = [n for n in z.namelist() if n.endswith('.nc')]
        for idx, name in enumerate(nc_inside):
            # Give it a unique name so we don't overwrite if multiple zips have same inner names
            extracted_name = filepath.replace('.nc', f'_part{idx}.nc')
            z.extract(name, '.')
            os.rename(name, extracted_name)
            extracted_files.append(extracted_name)
    os.remove(zip_path)
    return extracted_files

def process_city(city_name: str, config: dict) -> pd.DataFrame:
    nc_pattern = os.path.join(INPUT_DIR, f"era5_{city_name}_*.nc")
    nc_files = glob.glob(nc_pattern)
    if not nc_files:
        raise FileNotFoundError(
            f"ERA5 files not found matching: {nc_pattern}\n"
            f"Run 01_download_era5.py first."
        )

    print(f"\n[PROCESSING] {city_name} (found {len(nc_files)} original files)...")
    
    actual_files = []
    for f in nc_files:
        actual_files.extend(unzip_if_needed(f))
        
    dfs = []
    for f in actual_files:
        ds = xr.open_dataset(f)
        ds_mean = ds.mean(dim=["latitude", "longitude"])
        df_part = ds_mean.to_dataframe().reset_index()
        dfs.append(df_part)
        ds.close()
        
    df = pd.concat(dfs, ignore_index=True)
    
    # ── Group by time to merge variables from split NetCDF files (e.g. instant vs accum)
    if "time" in df.columns:
        df = df.groupby("time", as_index=False).first()
        df = df.rename(columns={"time": "timestamp"})
    elif "valid_time" in df.columns:
        df = df.groupby("valid_time", as_index=False).first()
